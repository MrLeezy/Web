...
async function getGenerationStatus(page) {
  return await page.evaluate(() => {
    ...
    // 3. 检查思考中文字
    const thinkingKeywords = ['正在思考', '思考中', '生成中', '正在生成', 'Searching', 'Thinking'];
    ...
    // 4. 检查完成标志
    const completeKeywords = ['思考完成', '已完成', '搜索完成', '搜索了', '个网页'];
...
async function detect(page, keyword, onLog) {
  ...
  try {
    ...
    if (pageContent.includes("登录") || pageContent.includes("Sign in") || pageContent.includes("请登录")) {
      onLog(`[${PLATFORM_NAME}] 需要登录，请在浏览器中登录...`);
      await sleep(15000);
    }

    // 开启深度思考
    onLog(`[${PLATFORM_NAME}] 检查深度思考状态...`);
    try {
      // 检查深度思考按钮是否已经打开
      // 已打开: class 包含 ds-toggle-button--selected
      // 未打开: class 只有 ds-toggle-button
      const isSelected = await page.evaluate(() => {
        const btn = document.querySelector('div[role="button"].ds-toggle-button:has(span:contains("深度思考"))');
        if (btn && btn.classList.contains('ds-toggle-button--selected')) {
          return true;
        }
        // 备用检查方式
        const allBtns = document.querySelectorAll('div[role="button"]');
        for (const b of allBtns) {
          if (b.textContent.includes('深度思考') && b.classList.contains('ds-toggle-button--selected')) {
            return true;
          }
        }
        return false;
      ...
      if (isSelected) {
        onLog(`[${PLATFORM_NAME}] 深度思考已开启`, "success");
      } else {
        onLog(`[${PLATFORM_NAME}] 正在开启深度思考...`);
        const deepThinkBtn = await page.$('div[role="button"].ds-toggle-button:has-text("深度思考")');
      ...
      onLog(`[${PLATFORM_NAME}] 开启深度思考失败: ${e.message}`);
    }
